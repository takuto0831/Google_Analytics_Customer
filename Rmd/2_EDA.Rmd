...
g3 <-
  ...
  ungroup() %>% 
  ggplot(aes(x=date, y=log1p(day_rev))) + 
  geom_line(color='steelblue') +
  geom_smooth(color='orange', method = 'loess', fomula = 'y ~ x') + 
  labs(
    x='',
    y='log1p(totalRevenue)',
...
g3 <-
  ...
  summarise(day_rev = sum(transactionRevenue)) %>% 
  ungroup() %>% 
  ggplot(aes(x=hour, y=log1p(day_rev))) + 
  geom_line(color='steelblue') +
  geom_smooth(color='orange', method = 'loess', fomula = 'y ~ x') + 
  labs(
    x='Hour of day',
...
- `browser`: top 9個にrevenue.
- `operatingSystem`: top 7個にrevenue.
- `isMobile`: 
- `deviceCategory`: 3種
...
g1 <-
  ...
  ungroup() %>% 
  top_n(9, rev) %>% 
  ggplot(aes(x=reorder(browser, -rev), y=log1p(rev))) +
...
g2 <-
  ...
  ungroup() %>% 
  top_n(7, rev) %>% 
  ggplot(aes(x=reorder(operatingSystem, -rev), y=log1p(rev))) +
  geom_bar(stat='identity', fill='steelblue') +
...
g3 <-
  ...
  ungroup() %>% 
  top_n(7, rev) %>% 
  ggplot(aes(x=reorder(deviceCategory, -rev), y=log1p(rev))) +
  geom_bar(stat='identity', fill='steelblue') +
