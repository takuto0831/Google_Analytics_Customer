...
```{r cache=TRUE}
# 訓練データ, テストデータ
train <- read_csv("~/Desktop/Google_Analytics_Kaggle/input/raw/train.csv",na = c("XNA","NA","","NaN","?")) %>% 
  mutate_if(is.character, funs(factor(.))) %>% 
  clean_names()  # change small letters

test <- read_csv("~/Desktop/Google_Analytics_Kaggle/input/raw/test.csv",na = c("XNA","NA","","NaN","?")) %>% 
  mutate_if(is.character, funs(factor(.))) %>% 
  clean_names()  # change small letters
```
